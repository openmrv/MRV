...
![Schematic view of a 2-dimensional spectral data space defined by two spectral bands.  Each dot represents a single pixel in an image. The location in the 2-d space of each pixel is defined its reflectance values in the two bands.](./figures/spectral_data_space.png){ width=50% }
...
![Classified spectral space. Each pixel from the prior figure has been labeled according to a classification scheme defined by the analyst.  In an ideal case such as that shown here, all of the pixels in each class can be grouped together into bounded regions.](./figures/spectral_data_space_and_classes.png){ width=50% } 
...
![A pixel in the image with spectral values that place it at the location indicated by "Pixel D" lands within the bounds of Class 3, and thus would be labeled Class 3.](./figures/spectral_space_classifier_new_pixel.png){ width=50% }
...
![Workflow of classification](./figures/WB_graphs_v2-03.png)
...
![This is how you do it](./figures/GEE_new_repo.png)

3. If you are a read on the openMRV group, you will see the script in "users/openmrv/MRV/LCLUC/cls_landsat_v2_colombia".  Or you can link to it directly using this link to the [GEE script](https://code.earthengine.google.com/96c7ffa0ea6df1c3fa967a76541dc99a)
...
![The Save As Function](./figures/GEE_save_as.png)
...
![Image of landsat composite](./figures/landsat8_composite.png)
...
![The training points from Module 1.2.1 displayed in the code editor of GEE.](./figures/training_points_colombia.png){ width=50% }
...
![An example of attributes listed in the Console of GEE for a feature collection containing the extracted values from the training points](./figures/feature_collection_with_loandcover_and_bands.png){ width=50% }
...
![An example of a classification tree approach to classification. The classifier identifies a value on one of the two axes that best separates the classes, with successive splits further isolating training points into classes.](./figures/CART_classification_cartoon.png){ width=50% }
...
![The view of the classified object as viewed in the Console of GEE](./figures/classifier_console.png){ width=50% }
...
![Image of classified map for all of Colombia](./figures/CART_classification_countrywide.png){ width=50% }
...
![Examples of classification problems, including missing pixels and over-prediction of developed areas.](./figures/classification_problems.png)
...
![Image of classification without 50% cloud cover threshold](./figures/CART_image_after_removing_image_cloud_filter.png){ width=50% }
...
![Image showing two-year composite](./figures/comparing_two_year_composite.png)
...
![Classification with two-year composite](./figures/cart_classifier_with_two_year_composite.png){ width=50% }
...
![Overclassification of the "developed" class near the Meta River in Northeastern Colombia. Colors and interpretation as in prior figure](./figures/figure_overclass_developed_plains.png)
...
![The result of the random forests classifier applied to the two-year composite for the same area shown above.  Note the substantial reduction in areas classified as developed.](./figures/randomforests_classifier.png)
...
![The Random Forests classification of the Guajira peninsula, showing that the developed class remains overpredicted.](./figures/guajira_rf.png)
...
![A classified output from the *k*-means unsupervised classification algorithm.  Note that the colors are randomly assigned, and thus have no inherent meaning.](./figures/figure_kmeans_examples.png)
...
For completeness, we have provided a full version of the Master script adapted to Mozambique.  It can be found in the OpenMRV directory under `cls_landsat_v2_mozambique`, or directly from [this link](https://code.earthengine.google.com/6ab10b7d479d7caa95b15019ff467949). 
...
![_fig_moz_initial_class](./figures/_fig_moz_initial_class.png)
...
![_fig_m2_zoom](./figures/_fig_m2_zoom.png)
...
As with Mozambique, we have provided a full version of the Master script adapted to Cambodia.  It can be found in the OpenMRV directory under `cls_landsat_v2_cambodia`, or directly from [this link](https://code.earthengine.google.com/0b2657db08572f234b4e6008d365285e). 
...
![fig_camb_fourmaps](./figures/fig_camb_fourmaps.png)
